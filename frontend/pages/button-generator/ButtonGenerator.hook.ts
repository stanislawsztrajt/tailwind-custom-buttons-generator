...
import { CREATE_COMPONENT } from "@pages/api/queries/component";
...
const useButtonGenerator = (component: Icomponent) => {
  ...
  const [isComponentBoxThemeDark, setIsComponentBoxThemeDark] = useState<boolean>(true);
  ...
  const postComponent = async () => {
    await Swal.fire({
      ...
      preConfirm: async () => {
        ...
        const { data } = await client.mutate<IcreateComponentResponse, Icomponent>({
        ...
        router.push(`/buttons/${data?.createComponent.data.id}`);
      },
    });
  };
