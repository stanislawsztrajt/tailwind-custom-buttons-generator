...
const ButtonsSearchResult: NextPage = () =>{
  ...
  const { data, loading } = useQuery<IcustomButtonsResponse, IcustomButtonSearchValue>(GET_CUSTOM_BUTTONS_BY_SEARCH_VALUE, { variables: { searchValue } })
          ...
          { data ? (
            <>
              { data.customButtons.data.length > 0 ? (
                ...
                <div className='mt-64 text-4xl text-center text-gray-300 lg:mt-80'>
                  Cannot find any buttons for <span className='font-semibold text-white'>{searchValue}</span>
                </div>
              ) }
            </>
          ) : null }
